...
* **速度快。** 明显快于 HaloPoint：单次散射场景每秒约 300–400 万条光线；两层全
  散射（`prob=1.0`，所有光线必经两次晶体交互）每秒约 40–60 万条。
...
> **运行时长。** 该示例追踪 9 段光谱 × 5 千万条光线。多核现代机器**约 2 分钟**；
> 较老笔记本可能更长。如仅做冒烟测试，可临时把配置中的 `"ray_num"` 改为
> `100000` 加速完成。
...
| [自适应亮度](doc/adaptive-brightness.zh.md) | 用户 | 自动 EV 归一化：算法原理、Off/On 双模式语义、XYZ 线性可加性不变量 |
| [Raypath 对称性](doc/raypath-symmetry.zh.md) | 用户 / 贡献者 | P、B、D 过滤器开关语义、D 启用条件、σ 推导 |

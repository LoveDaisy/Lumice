...
| `display_mode` | 通道计算显示模式（`normal` / `channel_br`）：commit 臂恒 `normal`（纹理只是线性 XYZ 源，预览 shader 在显示期自己算 B−R 灰度）vs export 臂写用户的值（CLI 在 `PostSnapshot` 对成品 sRGB 做同一公式 `src/util/channel_math.hpp`）。与 `tone` / `globe_back_fade` 同一形状：显示期参数，CLI 没有显示期阶段；机械权威是 `test_scene_commit_chain.cpp` 的 `kDivergingKeys` |
...
- **`sun_azimuth`（太阳方位角）**：两条臂都写 `0.0f`（`src/gui/file_io.cpp` 的
  `LUMICE_SceneSetLightSource` 调用）。⚠️ **与上面 `lens_shift` 那条性质不同，不要照抄它的措辞。**
  `lens_shift` 是**权宜**——"GUI 没有控件 ⇒ 没有可供 export 臂诚实描述的用户值，哪天加了控件这条
  注释就该停止是注释"。方位角是**主动固定**：GUI 的世界里太阳恒在方位角 0，用户转的是相机而不是
  太阳，这是同一张画面少一个自由度；CLI 保留手写非零值的自由度，因为手写 config 没有相机可转。
  ⇒ 这条注释**不在等一个控件**，将来也不会因为"加了控件"而失效。⛔ 不得据此给 GUI 加方位角控件。

  ⭐ **通则（owner 裁定，2026-09-09）：GUI 与 CLI 的能力不必逐项对齐。** 同族先例就在本文档里——
  GUI **永远只向 core 请求 dual equal-area 投影**，自己在前端做二次投影变换（见 §9.2 `lens` /
  `resolution` / `view` / `visible` 四行分叉的共同理由），而 CLI 可以直接指定任意投影。方位角与
  它是同一形状的决定，不是缺口。

  **导入侧的另一半**：`DeserializeFromJson` 读到非零 `scene.light_source.azimuth` 时**告警**
  （`WarnUnsupportedByDesign`），而不是静默把天空转回去。同一个 helper 也守着另外两个同型字段——
  `render[].lens_shift`（即上面那条例外的导入侧）与 `scene.geom_clock`。判据是三条同时成立：
  **GUI 数据结构里没有字段能装它** + **导入路径今天完全静默（连日志都没有）** + **非默认值能改变
  画面或仿真**。⚠️ 这三条不是"把 core 的键都补上告警"——`render[].overlap` 之类"从未有过用户可设
  对应物"的键不满足第三条，`render[].grid.*` 这类"GUI 能表达、只是这条导入路径没接线"的键不满足
  第一条，都不在此列。

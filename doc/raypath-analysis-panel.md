...
> C API 追加到 v4.30），已随 PR #347 合入 `main`。
> 本文是 (i)「识别」已交付形态的机制记录；「光路分析」整体为什么拆成 (i)/(ii)、(ii) 为什么搁置、
> (i) 路线接下来往哪走，见功能总篇 [`raypath-analysis.md`](raypath-analysis.md)。
> 本文档结构与 §1/§2 的裁决叙事保持设计阶段原文，§3–§5、§7 的机制描述、§6 诚实边界、§8 开放
> 设计点均已回写为实现后的状态（章节相对设计阶段草稿有重排：新增 §7「结果条目的可打印字符串」
...
一条全分辨率通道」这个**形态**本身（规模量级见 [`raypath-analysis.md`](raypath-analysis.md) §4.1）。
...
1. **后端：v1 只走 CPU 路，且必须显式声明，不是静默 fallback。**
   ...
   **多 worker（as-built，2026-09-13 起）**：CPU 路的分析本来就跑在多 worker 上；GPU 偏好的
   ...
   （`analysis_pool_simulators_`，`num_workers > 0 ? num_workers : min(<平台核数>, <平台上限>)`，两者均由
   `ServerImpl::AutomaticWorkerBaseAndCap()` 一并返回，核数来源与上限按平台分档、与 CPU 路同一规则
...
(ii)「每桶真实辐射量、随即时开关变化」的五条实现路线**仍然搁置**，本形态不解除这个搁置；它改变的
是重启前要问用户的那个问题——从「你需要多大的 K」变成「有了分析面板 + 排除后重跑，还需要即时
开关吗」。五条路线的规模事实、被否决方向、重启条件与探针，完整记录在
[`raypath-analysis.md`](raypath-analysis.md) §4。

...
| `LUMICE_CPU_HANDOFF_BATCHES` | `env::CpuHandoffBatches` ← `server.cpp` `GenerateScene` | CPU 路一次队列交接携带多少份 SimData（= 物理 batch 数 × 波长数；默认 `kCpuHandoffBatches`=16，离散光谱按波长数相应减少物理 batch 数；1 = 回到每个物理 batch 交接一次的旧行为，兼作逃生口）。物理 batch 大小仍由 `LUMICE_DISPATCH_RAY_NUM` 决定；机制与取值依据见 `doc/performance-testing.md`「CPU worker handoff grain」 | 无（纯实验旋钮，启动 INFO 可观测） |
| `LUMICE_GEOM_CLOCK` | `env::GeomClock` ← `simulator.cpp` `Run` | legacy CPU 几何时钟 K = 每个采样形状服务多少光线。1 = 每光线一个新形状；默认 32 = `kSmallBatchRayNum`（该值是光线分批 stride，几何重采样搭它顺风车，**并非为采样质量选定**）。⚠️ **曾经有一条「超出 SimBatch 大小就破坏堆」的安全域上界，现已消除**（默认 dispatch 下实测：修复前 128/256/1024/32768 各 3/3 崩，修复后全部 EXIT=0）——那不是本旋钮的属性，而是 hit-loop 缓冲对按 batch 定容、余量为 0；现由 `simulator.cpp` 的 `ResetHitLoopBuffers` 按 `buffer_data[0]` 的容量定容，容量模型不再读 `geom_clock`。**仍然成立的是一条语义上界**：子批受 SimBatch 约束（`curr_ray_num ≤ crystal_ray_num[ci] ≤ ray_num`），所以把本旋钮调过 dispatch 只是不再起作用（要扫到 GPU 量级的几何时钟，仍须同时抬高 dispatch，例如 dispatch=65536 时 K=32768 才有 13 个不同形状）。前后对照与机制见 `env_knobs.hpp` `GeomClock()` | 无（纯实验旋钮，启动 INFO 可观测） |
| `LUMICE_GPU_GEOM_CLOCK` | `env::GpuGeomClock` ← 两 GPU 后端 per-batch pool build | GPU 几何时钟 K = 每采样形状服务多少光线。默认 0 = disabled = 今天行为（每 batch 单形状，P=1，K=D）；正值 K 触发 K-shape 池，每 batch 每 `(layer,ci)` 建 `P_ci=ceil(N_ci/K)` 个形状。**与 `LUMICE_GEOM_CLOCK`（legacy CPU only）独立**——同义 "rays per shape" 但走不同代码路径、有各自的容量模型（GPU 侧见 `P_max` 推导，详见 seam-design.md §8 P2）。**仅影响 Metal/CUDA 两 GPU 后端**（CPU-legacy 用独立的 `LUMICE_GEOM_CLOCK`）。现由 config 提供生产默认值：config 未设时默认仍 0（disabled/opt-in）；env 现在覆盖的是 config 解出的默认值，而非硬编码 0（与 `LUMICE_DISPATCH_RAY_NUM`/`LUMICE_COMMIT_RAY_NUM` 同款分层模式）。安全域 `{0} ∪ [1, 64]`（`kGeomClockMax`；≥128 破堆） | **config `scene.geom_clock` JSON 字段**（env 仅作 debug/CI 覆盖，生效 INFO 可观测） |

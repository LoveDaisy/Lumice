...
| `LUMICE_CPU_HANDOFF_BATCHES` | `env::CpuHandoffBatches` ← `server.cpp` `GenerateScene` | CPU 路一次队列交接携带多少个物理 batch（默认 `kCpuHandoffBatches`=16；1 = 回到每个物理 batch 交接一次的旧行为，兼作逃生口）。物理 batch 大小仍由 `LUMICE_DISPATCH_RAY_NUM` 决定；机制与取值依据见 `doc/performance-testing.md`「CPU worker handoff grain」 | 无（纯实验旋钮，启动 INFO 可观测） |

...
叠加层（网格、圆、标记、标签、镜头边框）以各自原色画在诊断图之上。诊断图开启时，背景照片与光路染色的 Colored 合成图会被隐藏（它们的设置保留不变）；在 **Mode ▶ Print** 下该选项变灰：print 没有独立的红、蓝通道可减。截图默认导出的就是屏幕上的画面，也可以在截图的选项窗口里只为这一张图切换到另一种显示模式（见 [§6](#6-保存与重载)）；导出的 config 带 `"display_mode": "channel_br"`，CLI 渲染出同样的灰度图。
...
要保存画面本身，用 `Save ▶ Screenshot...`。它在询问文件名之前会先弹出一个选项窗口，按预览当前显示的内容预填：每种叠加层（地平线、网格、镜头边框、两族角距圆、天空参考点）各有一个 **Line** 框和一个 **Label** 框，另有显示模式（Normal / Channel B−R）。取消勾选这一张图不想要的内容——比如只去掉网格的数字、保留网格线。预览上没有显示的叠加层会置灰：选项窗口只能做减法，想让某个叠加层出现在图里，请先在 Overlay 面板打开它。选择只对这一次导出生效；面板与预览保持原样，下一次 Screenshot 仍从屏幕所示开始。

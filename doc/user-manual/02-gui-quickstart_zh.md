...
**Display ▶ Show As ▶ Channel B-R** 把预览变成一张颜色诊断图：每个像素显示正常画面的蓝通道减红通道（显示用的 sRGB 值，gamma 之后——与在图像编辑软件的通道计算里做减法相同），以灰度表示。中灰表示没有差别；比中灰亮偏蓝，比中灰暗偏红。纯蓝读作白、纯红读作黑，任何中性像素——包括空天空和镜头圆之外——都读作中灰。切回 **Normal** 即恢复正常画面；两个方向都不会重跑模拟。

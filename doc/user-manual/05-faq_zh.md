...
2. **噪点多？** 升 `scene.ray_num`，或者把 spectrum 收窄一点（如果色散噪声占主导）。若只是某一个晕的边缘比画面其余部分粗糙（弱晕挨着亮晕），确认 `scene.ray_allocation` 是 `"adaptive"`——GUI 里就是 Settings 里的 **Adaptive ray allocation** 设置，默认开启。

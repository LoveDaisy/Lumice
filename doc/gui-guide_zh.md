...
- **预览纹理**：可选，保存最近一次渲染结果。自格式 v5 起存的是**未曝光的线性 XYZ 能量**
  （与实时预览上传的浮点数据相同），以 zlib 压缩的 float32 形式存放，前面带一个小头部记录该帧的
  曝光测量值。因此重开的文档走与实时运行完全相同的 shader 路径——EV 滑杆、曝光模式、背景色、
  print 模式在重开文档上照常生效，像素里也不再烘焙任何曝光时的 clip。v4 及更早版本写出的文件
  内嵌的是 8-bit PNG（曝光已烘焙），仍可打开，但在下一次运行之前保持其烘焙时的曝光。

格式头部为 44 字节，包含魔数 `LMC\0`、版本字段、flag 位（是否带纹理、纹理用哪种编码），以及 JSON / 纹理两段载荷的偏移量与大小。值以人类可读的语义类型存储（如 `"prism"` 而非枚举索引），保证前向兼容性。

由于 v5 纹理是无损浮点数据，项目文件比以前大：默认 1024 仿真分辨率下，稀疏场景约 2–3 MB、
密集全天场景约 20 MB（2048 分辨率下再乘 4），旧 PNG 格式为 0.2–5 MB。若文件体积比内嵌预览
更重要，可在 Save 菜单里取消勾选「Include Texture in .lmc」。

...
不含魔数、在 Mac 与该机上都自洽的定义。

同一台机的原生 Windows（home-win，MSVC shared 构建，W=10，预算 = 16 − 10 = 6，12 s 窗口，n=5，CoV ≤ 0.4%）：

| 臂 | 2 s 稀疏轮询 | 20 ms GUI 轮询 | 损失 | 20 ms 下每次 poll 周期 |
|---|---|---|---|---|
| 全核池（修复前） | 5.99 M rays/s | 5.22 | 12.9% | 32 ms |
| 强制串行 | 5.91 | 5.77 | 2.4% | 93 ms |
| **预算 = 物理核 − W = 6** | 6.00 | 5.58 | **7.0%** | 41 ms |

**三平台合读。** 修复把 20 ms 轮询下的 worker 吞吐损失从 18.0 / 17.3 / 12.9%（Mac / Linux / Windows）降到
11.3 / 12.8 / 7.0%，收回回退的 55–65%；相对同日强制串行臂仍多 1.5 / 3.8 / 4.6 pp，20 ms 绝对吞吐为串行臂的
97.0 / 96.4 / 96.7%。换来的是 `PostSnapshot` 在 GUI 节奏下的周期 83 / 39 / 41 ms 对串行的 118 / 88 / 93 ms
（1.4 / 2.2 / 2.3×）。立项时定的验收线是「损失 ≤ 串行 + 2 pp 且绝对吞吐 ≥ 串行 98%」，三台机器都没到；数据说
这条线在「起任何池」的前提下就不可达——池线程与 worker 抢的是同一批核，多一个线程就多一份损失，剩下的只有
「要多快的帧」对「肯付多少 worker 吞吐」的取舍，这一步按「物理核 − W」落地并把三台机器的完整曲线留在这里
供裁决（Linux 五臂表里固定预算 2 的那一行是另一个候选点）。

...
- `ServerImpl` 构造函数在算出 `worker_count` 的同一处算 CPU 路预算
  `render_thread_budget_ = min(max(0, PhysicalCoreCount() − worker_count), kRenderThreadBudgetCap)`、
  GPU 路预算 `render_thread_budget_ = max(0, PhysicalCoreCount() − worker_count)`（不封顶，`src/server/server.cpp`），
  算一次、存成员——`worker_count` 在一个 server 实例的生命周期内是常量，没有 resize 路径，所以预算也是。构造行日志
  ...
  物理核数 − 1（**不封顶**，见下方"封顶"条），GUI 在 GPU 路上仍拿到接近全核的并行。常备的 CPU 分析池不计入：它只在
  分析会话时唤醒，而分析会话与渲染会话在同一个 server 里互斥。**是物理核不是 `hardware_concurrency()`**：这一步
  最初按逻辑核写（立项公式原文如此），在 16C/32T 的参照机上 `32 − 10 = 22` 个池线程量出的损失与全核池**完全相同**
  （下表 17.3% vs 17.3%）——多出来的线程落在 worker 所在物理核的 SMT 兄弟线程上，抢的是同一执行单元；
  换成 `16 − 10 = 6` 才把损失从 17.3% 拉到 12.8%。无 SMT 的机器（Mac）两种算法相等。
- **封顶（`kRenderThreadBudgetCap = 2`，CPU 路专属）**：三平台复测显示"物理核 − worker_count"无上限仍未达到本节
  立项时定的验收阈值（见下方三平台合读），owner 在三条候选路线里先选择"加上限"落地：即使一个物理核确实空闲，
  一个落在它上面的 `thread_budget` 大小的池仍然会可测量地拖慢 CPU 路自己的 worker——这不是 SMT 假空闲的
  问题（那个已经被"改用物理核"解决），而是池本身的调度/上下文切换开销随它的线程数增长。home-wsl 上把预算从
  6 降到固定 2 把 20 ms 损失从 12.8% 拉到 11.5%，代价是 `PostSnapshot` 自己的周期从 39 ms 变慢到 59 ms（下表
  Linux 五臂表最后一行）。2 不是另外拍的数字：它就是 `ParallelRows` 自身"预算 < 2 才划算内联"门槛的下界，即
  这个封顶不会比"永远不起池"更慷慨，只是在门槛之上截断。**这个封顶只施加在 CPU 路**：GPU 路的单个 CPU
  worker 不做追光（GPU 在做），它的空闲 CPU 核不是在保护一个吃满 CPU 的仿真，本节顶部的 AC1 明确要求 GPU 路
  保留接近全核并行（`hw−1`）——不分路封顶会让 GPU 路的 GUI 并行度从 phys−1 掉到 2，直接违反这条要求，也没有
  任何针对 GPU 路的测量支持这个代价。
...
`test_server_render_thread_budget.cpp` 在 CPU 路 `worker_count ∈ {1, phys−2, phys, phys+3}`（第一档钉住
封顶本身、其余三档钉住 `max(0, ·)` 钳位或与封顶重合的边界）与真实 GPU 路（`worker_count=1`，断言
`phys−1` 且不受封顶影响，AC1 的直接回归目标）上断言公式；
...
| 预算 = 物理核 − W = 6（无上限，code-review round 1 之前的实现） | 12.73 | 11.10 | 12.8% | 39 ms |
| 预算固定 2（对照，临时改常量） | 12.76 | 11.29 | 11.5% | 59 ms |
| **预算 = min(物理核 − W, 2) = 2（本节定稿，`kRenderThreadBudgetCap`，生产代码实测）** | 12.90 | 11.46 | **11.2%** | 59 ms |

读法：在 SMT 机上「逻辑核 − W」等于没修；「物理核 − W」把损失从 17.3% 降到 12.8%，比串行多 3.8 pp，换来
`PostSnapshot` 周期 88 → 39 ms（2.2×）。封顶到固定 2 再少约 1.6 pp 损失但周期慢到 59 ms——池大小与 worker
损失之间是连续的取舍，没有一个预算能同时拿到串行的损失和并行的周期。最后一行是 `kRenderThreadBudgetCap`
落地后用生产代码（而非临时改常量）重新测的数字，与上面的对照行一致（11.2% vs 11.5%，同一测量口径下的
正常波动），确认「封顶到 2」在真实代码路径上复现了对照行的效果。**这一行相对同日强制串行臂损失多
11.2% − 8.0% = 3.2 pp，20 ms 绝对吞吐 = 11.46/11.76 = 97.4%（本节末尾"三平台合读"给出立项阈值下的判读）。**

同一台机的原生 Windows（home-win，MSVC shared 构建，W=10，12 s 窗口，n=5）：

| 臂 | 2 s 稀疏轮询 | 20 ms GUI 轮询 | 损失 | 20 ms 下每次 poll 周期 | CoV |
|---|---|---|---|---|---|
| 全核池（修复前） | 5.99 M rays/s | 5.22 | 12.9% | 32 ms | ≤0.4% |
| 强制串行（首轮） | 5.91 | 5.77 | 2.4% | 93 ms | ≤0.4% |
| 预算 = 物理核 − W = 6（无上限，code-review round 1 之前的实现） | 6.00 | 5.58 | 7.0% | 41 ms | ≤0.4% |
| 强制串行（本轮，与下一行同批交错） | 5.93 | 5.77 | 2.7% | 93 ms | ≤0.5% |
| **预算 = min(物理核 − W, 2) = 2（本节定稿，生产代码实测）** | 6.01 | 5.79 | **3.6%** | 62 ms | ≤0.5% |

读法：封顶到 2 在 Windows 上比「物理核 − W = 6」损失从 7.0% 大幅降到 3.6%——降低池线程数在这台机器上的
边际收益比在 Linux 参照机上更大（6.0 pp vs 1.6 pp）。相对本轮同日串行臂只多 **0.99 pp**，20 ms 绝对吞吐
达到串行臂的 **100.3%**（本轮串行 20ms 5.773M，封顶臂 20ms 5.788M——封顶臂比串行臂本身还快，在测量噪声
范围内可读作"追平"）。这是三台机器里唯一在两条判据上都清过立项阈值（≤ +2pp 且 ≥ 98%）的一组。

**三平台合读（`kRenderThreadBudgetCap=2` 落地后）。** 相对同日强制串行臂的损失差与 20 ms 绝对吞吐比：

| 机器 | 封顶后损失 | 同日串行损失 | Δ（封顶 − 串行） | 20 ms 绝对吞吐 / 串行 | 判读（≤2pp 且 ≥98%） |
|---|---|---|---|---|---|
| Mac（12 核，无 SMT，预算天然 = 2，未受本次封顶影响） | 11.3% | 9.8% | +1.5 pp | 97.0% | 差 1.0pp 未过 |
| Linux（16C/32T，本节） | 11.2% | 8.0% | +3.2 pp | 97.4% | 未过 |
| Windows（16C/32T，本节） | 3.6% | 2.7% | **+1.0 pp** | **100.3%** | **过** |

Mac 的数字不因本次封顶而改变：这台机器 `worker_count=10` 时物理核 − W = 2，本来就等于封顶值，`min` 是恒等
运算。三台机器里 Windows 完全清过立项阈值，Mac 只差 1 个百分点（绝对吞吐 97.0% vs 98%），Linux 差距最大
（20 ms 绝对吞吐 97.4%，损失差 +3.2pp）——三者差距都在个位百分点，与探针本身 1–4% 的 CoV 同量级。换来的是
`PostSnapshot` 在 GUI 节奏下的周期，封顶后 Mac/Linux/Windows 分别是 83/59/62 ms（对无封顶时的 83/39/41 ms 与
纯串行的 118/88/93 ms）——即封顶用"更慢一点的帧"（Linux/Windows 周期都变慢约 1.5×）换到"更接近串行的
worker 吞吐"，但换到的吞吐收益 Windows 明显更大（损失少掉 6.0pp 对 Linux 的 1.6pp）。
立项时定的验收线是「损失 ≤ 串行 + 2 pp 且绝对吞吐 ≥ 串行 98%」：**候选 (ii)（本节）让 Windows 完全达标，
Mac/Linux 仍未达标但差距已缩小到 1–3 个百分点**；候选 (i) 的完整三平台数字见上面两张表标注"无上限"的那两行。
**owner 终裁（2026-09-20）：接受候选 (ii) 的残余**——`kRenderThreadBudgetCap = 2` 维持，Mac +1.5 pp / Linux +3.2 pp
相对纯串行的差距视为已接受的代价（换 `PostSnapshot` 1.7–2.2× 于串行）。owner 同时声明这是**基于现有数据的判断**：
若 owner 本人或内测用户发现新的情况（例如某台机器上 GUI 运行中的仿真吞吐损失明显超出本表），本节重新讨论，
候选 (iii) 的开关就是把该常量改为 0。

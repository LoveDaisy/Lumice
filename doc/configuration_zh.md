...
>
> - **只有随机取向晶体**（22° / 46° 晕，大多数「随机」场景）：晕的形状与相对亮度不变。默认的
>   `ev_mode: relative` 下画面看起来一样，只是同一 `ray_num` 下更嘈杂——约一半光线在入射时被拒，
>   把 `ray_num` 翻倍即可回到原来的噪声水平。`ev_mode: absolute` 下同一 EV 画面暗一档（EV 加 1，
>   或 `intensity_factor` ×2，即可对齐旧图）。
> - **定向晶体**（片状、柱状、Parry / Lowitz 取向）：亮度*分布*会变，因为正对太阳的取向现在比不正对
>   的取向权重大。变化多少取决于晶体形状、倾角离散度与太阳高度：倾角离散度小的水平片状晶体与此前
>   相差不大（每个被抽到的取向对太阳呈现的面积几乎相同，加权接近均匀）；倾角离散度大的片状/柱状
>   晶体，或同一层混合定向与随机取向晶体的场景，会有肉眼可见的变化。`ev_mode: absolute` 下这类
>   场景也会变暗，视形状与太阳高度约 0.7–1.4 档（实测值见 `doc/ev-pipeline-architecture.md` §7.2）。

...
| `upper_indices` | 整数数组 | 否 | [1,0,1] | 上锥面的 Miller 指数 `(h, k, l)`——必须恰好三个整数、`k` 必须为 `0`，详见下方[读懂 Miller 指数回退警告](#12-读懂-miller-指数回退警告) |
| `lower_indices` | 整数数组 | 否 | [1,0,1] | 下锥面的 Miller 指数 `(h, k, l)`——规则同上 |
...
| `max_hits` | 整数 | 是 | - | 一条光线在一块晶体内最多与多少个面发生作用，**含入口面**：光线在第 `max_hits` 个面之后被丢弃，其面序列长度至多为 `max_hits`（`max_hits = 1` 只发出入口面的外反射）。三个后端（legacy CPU / Metal / CUDA）预算相同。范围 `[1, 64]` |
| `ray_allocation` | 字符串 | 否 | `"proportional"` | 每个散射层的光线如何在各条目之间分配。`"proportional"` 按 `proportion` 分配——采样份额与晶体个数份额是同一个旋钮。`"adaptive"` 在渲染进行中从主跑自己的每一批（在哪个后端渲染就在哪个后端统计）在线累计各条目的每光线能量，据此持续更新每条目的采样份额 `q_i ∝ p_i·√E[e²]`（Neyman 分配），按 `q_i` 分配光线，并把落入条目 *i* 的每条光线权重乘以 `(p_i/ΣP)/(q_i/ΣQ)`，因此**期望图像不变**，只有方差分布改变。见下方说明。 |
...
>
> **计入 `ray_num` 的光线并不都以全权重进入晶体。** 分给某颗晶体的光线进入晶体时，权重乘以
> `A/(S/2)`（晶体沿光线方向的投影面积除以其表面积的一半）。每条光线都被追迹，所有路线（CPU、Metal、
> CUDA）都一样，没有光线被丢弃。光线在入射处减去的权重仍计入发射能量，每条发放的光线都计入
> `ray_num`。对随机取向的晶体该因子平均恰为 1/2。这对
> `proportion` 意味着什么见下方 `ray_allocation` 说明，它带来的变化见其后的迁移说明。

> **`ray_allocation` 决定一个条目分到多少光线；`proportion` 仍然只说它代表多少颗晶体。**
> 散射条目的 `proportion` 是*等表面积约定下的晶体个数份额*：该条目在本层晶体中所占的个数比例，
> 且每颗晶体都按相同的总表面积计。Lumice 的晶体形状没有绝对尺寸，「多少颗晶体」必须先定一个尺寸
> 约定才有意义，追迹器实现的就是这一个——一条光线被采样到的晶体截获的概率是 `A/(S/2)`，即晶体沿
> 光线方向的投影面积除以其表面积的一半（作为权重施加在每条光线上，见上一条说明）。对随机取向的晶体，这个个数份额也恰好就是该条目的
> *能量份额*（Cauchy 公式：随机取向凸体的平均投影面积为 `S/4`，因此每种形状截获的都是分给它的
> 光线中相同的比例）；对定向晶体（片状、柱状）二者不再相等——侧立的片状晶体截获的光比其个数份额少。
...
> 导出的 config——而那里的**文档**默认是 `adaptive`（新文档、该设置出现前保存的 `.lmc`、不带该键导入的
...
> **迁移说明（投影面积入射接受，2026-09）。** 此前分给晶体的每条光线都会进入晶体，不论晶体以何种
> 朝向面对太阳，于是侧对太阳的片状晶体与正对太阳的同一片晶体截获同样多的光。现在它按自身呈现的
> 投影面积截获光线，与真实天空一致。晶体*内部*的光线追迹完全不变；变的是每种取向贡献多少。
>
> - **哪些画面会变、变多少**只由一件事决定：晶体在其被采样的各个取向下对太阳呈现的面积变化有多大。
>   变化小，加权就接近均匀，画面几乎不动；变化大，对太阳呈现面积大的取向就占主导，各晕之间的相对
>   亮度随之移动。几乎不受影响：倾角离散度小的水平片状晶体（常见的幻日 / 环天顶弧场景——每片被抽到的
>   晶体对太阳呈现的面积几乎相同），以及随机取向的近等轴柱（`height` 约为 1；从端面到侧面面积约差
>   1.8 倍）。会有可见变化：随机取向的薄片（面积约差 7.5 倍）、长柱、倾角离散度大或太阳高度低的定向
>   晶体，以及同一层混合定向与随机取向晶体的场景（各晶体的相对份额会移动）。
> - **噪声**：没有光线被拒——每条都以权重 `A/(S/2)` 追迹——因此画面背后的光线数不会减少。晶体对太阳
>   呈现的面积在其采样取向间变化越大，权重随之变化越大，同一 `ray_num` 下会多出一些逐像素噪声；单波长
>   实测，这约为「以概率 `1 - A/(S/2)` 丢弃光线」这一替代做法逐像素方差的一半。
> - **整体亮度**：默认的 `ev_mode: relative` 下按新画面重新测光，不会变暗。`ev_mode: absolute` 下光线在入射处减去的权重仍计入发射，
>   同一 EV 画面会变暗：随机取向晶体恰好暗一档（EV 加 1，或 `intensity_factor` ×2，
>   即可对齐旧图），定向晶体视形状与太阳高度约暗 0.7–1.4 档（实测值见 `doc/ev-pipeline-architecture.md` §7.2）。
> - **`proportion`** 数值不变，但语义现在是精确的：等表面积约定下的晶体个数份额（见上一条说明）。
>   对随机取向晶体它等于该条目的光量份额；对定向晶体则不等。
> - **光路分析**：分析窗口里的能量份额出于同一原因会变化（`doc/user-manual/06-raypath-analysis_zh.md`）。
>
> 没有任何 config 键变化，也没有 config 需要修改才能继续加载。凭肉眼对照旧渲染调出来的 config——
> 尤其是混合定向与随机晶体的——可能需要重新审视其 `proportion`。
...
|------|------|------|--------|------|
| `crystal` | 整数 | 是 | - | 晶体ID引用 |
| `proportion` | 浮点数 | 否 | 100.0 | 本层内的晶体个数份额，按等表面积约定计（见 `ray_allocation` 说明） |
...
| `globe_back_fade` | 浮点数 | 否 | 0.0 | 仅 `globe` 镜头（其余镜头忽略）：球的**背面**的雾化特征长度——背面透过正面看到、随离相机的距离像雾一样渐隐，权重为 `exp(−depth / globe_back_fade)`：值越大衰减越慢，且永远不会精确降到 0。单位与 globe 自身一致（单位球，相机距离 `D = 4`），背面最深处（球心正后方）比轮廓远 `(D+1) − √(D²−1) ≈ 1.127`；GUI 滑杆范围 0–2，0.3–0.6 是一层淡雾，取 2 时最深处已有约 0.57 的权重。`0` 只显示朝向相机的半球，即本键出现之前的画面。负值按 0 处理。详见下方 `globe` 镜头一节。 |
| `display_mode` | 字符串 | 否 | `"normal"` | 成品图**以什么形式显示**。`"normal"`：图像本身。`"channel_br"`：「这里偏蓝还是偏红」诊断图——每个像素变为显示用 sRGB 通道（gamma 之后，即图像软件「通道计算」做的减法）的灰度 `clamp(0.5 + 0.5·(B − R), 0, 1)`：中灰 = 无差异，更亮 = 偏蓝，更暗 = 偏红。该值随曝光变化，且 R 或 B 触顶处被压平，比较不同位置须在同一 EV 下。`"tone": "print"` 下不生效；开启时染色场景不产出光路合成图，非默认 `ray_color` 会让诊断失去意义（显示的是染色的固定偏移）；二者均在 commit 时记警告。叠加层照常画在其上。未知取值按 `"normal"` 处理并警告。公式：`src/util/channel_math.hpp`。 |
...
- **背面**（`globe_back_fade`）：每个像素的视线与球相交两次。默认只显示近交点；`globe_back_fade > 0` 时，远交点的光以**叠加**方式加到该像素上（晕是发光体，正面不遮挡背面），权重为 `exp(−depth / globe_back_fade)`（指数雾），`depth` 是远交点比轮廓离相机多出的距离。权重在球缘处为 1，背面与正面连续衔接，在 `depth = globe_back_fade` 处为 `e⁻¹ ≈ 0.37`，且永远不会精确到 0——刻意不设截止。背面的**线**——网格（`elevation` / `longitude`）、`horizon`、`angular_dist` 与 `view_dist`——也画在远交点处，每条线的不透明度乘以同一权重，并且位于正面线之下：两者重叠的像素上正面线胜出。marker（`zenith_nadir`、`markers`）与文字标签仍只画在正面。`visible` 与 `front` 对两个交点**各按其自身方向**裁剪：同一像素的近、远交点一般位于不同高度，因此在 `visible: upper` 下，位于地平线以下的远交点既不加光也不画线——即使它前面的近交点保留；位于地平线以上的远交点则照样透过被裁掉的近交点显示。天空 `background` 属于正面：近交点被裁掉的像素上，远交点的光画在黑色（`tone: print` 下为空白 `paper`）之上，而不是天空色。原始 XYZ 导出（`--format npy`、C API）不是显示，始终保留两侧能量。已知缺口：开启 `raypath_color` 时，合成图仍只按近交点裁剪 globe 像素。曝光不受影响（背面的光不计入落地能量）。GUI 预览与 CLI 使用同一权重，公式位于 `lm_proj::GlobeBackFadeWeight`（`src/core/shared/projection_shared.h`）。
- `.lmc` 兼容性：唯一新增字段是 `globe_back_fade`（默认 0），旧 `.lmc` 加载行为不变。
...
- `crystal[].shape.upper_indices` / `crystal[].shape.lower_indices` 必须恰好为 3 个整数（如果指定）。
  长度不是 3 的数组会被报出来、并保留 wedge 角原有的取值，而**不是**静默跳过。完整规则与每种拒绝的
  表现形态见[读懂 Miller 指数回退警告](#12-读懂-miller-指数回退警告)
...
- **wedge 角合法性**：一侧锥的 wedge 角——直接给定的 `upper_alpha`/`lower_alpha`，
  ...
  锥或棱柱段——仍能凑够面数，形状照常构建）。这种静默只发生在直接给 `upper_alpha`/
  `lower_alpha` 的路径上：由 Miller 指数推出的越界角在成为角度之前就被拒绝，并且会报出来
  ——见[读懂 Miller 指数回退警告](#12-读懂-miller-指数回退警告)。

  **这个角是从哪儿量起的。** wedge 角是锥面与 **c 轴**的夹角，等价地说，是锥面与它所升起的
  那个棱面（侧面）之间的夹角；它**不是**锥面与基面（顶/底面）的夹角。两者极易混淆，因为
  晶体学表格通常给的是与基面的那个角。冰的经典锥面 `{1, 0, -1, 1}` 在文献里记作与基面成
  62.0°，同一个面在这里是 `upper_alpha = 28.0°`——因为 90° − 62.0° = 28.0°。直接从表格抄来
  而漏掉这一步减法不会报错，只会构建出一个明显不同的晶体，所以值得专门说明。
...
**错误描述**：`upper_indices` / `lower_indices` 指定的是晶体锥面所在的那个 Miller 面，
当它们无法指向一个真实的面时，会被整体拒绝，而不是取其中一部分来用。

这三个整数是 `(h, k, l)`——约简后的三指数记法，**不是**把四指数 Miller-Bravais 标号
去掉一位。GUI 的晶体编辑器显示的是四指数形式 `{h, k, i, l}`（冰的默认值读作
`{1, 0, -1, 1}`），其中第三个数是**推导出来的、并不单独存储**：`i = -(h + k)`。
所以在这里写四个数并不是"同一件事写长一点"，而是多写了一个数，会被拒绝。由于默认值的
`k = 0`，`i` 恰好等于 `-h`，两种记法的数字集合看起来一样、只是顺序不同——这正是这个错误
既容易犯、又不容易被发现的原因。

以下情况会被拒绝：

- 数组不是恰好 3 个整数——包括从 GUI 抄来的四指数标号，以及只写了一半的三元组；
- `k != 0`——相对棱边转过一定角度的锥面不是本晶体模型能表达的形状，因此没有可换算的角度；
- `h` 或 `l` 为负；
- 任一元素不是整数；
- `h` 与 `l` 各自合法，但二者之比得到的角度落在 `[0.1°, 89.9°]` 之外——这样的角构建不出任何面，
  见上方 [pyramid 形状合法性](#pyramid-形状合法性)。
...
要的那个晶体。正确写法是三个非负整数且 `k = 0`：

```json
{
  "upper_indices": [1, 0, 1],    // 正确：(h, k, l)，k = 0 -> upper_alpha = 28.0 度
  "lower_indices": [1, 0, -1, 1] // 错误：GUI 的四指数标号，应去掉推导出的 i = -(h+k)
}
```

直接给 `upper_alpha` / `lower_alpha` 始终是另一条可选路径——两者同时存在时，显式角度优先，
指数根本不会被读取。
...
内置预设的 zenith 分布有两个可覆盖的面，各占一个键、各自独立判定，且都被约束在该预设既有的分类判据容差域内：

- `zenith_std`——数字。越界值会被 clamp 到边界而不是被拒绝，clamp 行为会被上报（见下）。
- `zenith_type`——`.lmc` 的 axis 块所用的分布类型名之一（`gauss`、`gauss_legacy`、`laplacian`、`uniform`；Lowitz 另接受 `zigzag`），限定在分类器本来就把该预设归入的那一族。离散集合没有"最近的合法值"，所以不在集合内或无法识别的 type **不做 clamp**：它被忽略、内置 type 继续生效，并上报这次拒绝。同一节点里被拒绝的 `zenith_type` 不会拖累合法的 `zenith_std`。

没有可调面的预设（例如全均匀分布的预设）会直接拒绝任何存入值。缺键即用内置值；加入 `zenith_type` 时文件的 schema 标记没有变，因为没有任何既有键改变含义。

#### 降级输入的处置

文件本身无法解析、字段类型不符、预设值非数字或非有限、预设值被 clamp、预设 type 被拒绝，这几种情况都归入同一个提示通道，而不是散落成若干条各写各的警告，方便用户在一处看全所有被忽略或调整的内容。字段级类型错误会丢弃整份覆盖文件的 GuiState 半区（不会出现只应用一半的结果）；`presets` 子树的解析与之独立。

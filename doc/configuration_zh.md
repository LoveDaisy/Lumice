...
| `globe_back_fade` | 浮点数 | 否 | 0.0 | 仅 `globe` 镜头（其余镜头忽略）：球的**背面**在轮廓之后多深的范围内仍然可见——透过正面看到、随离相机的距离渐隐。单位与 globe 自身一致（单位球，相机距离 `D = 4`），背面最深处（球心正后方）比轮廓远 `(D+1) − √(D²−1) ≈ 1.127`；`0` 只显示朝向相机的半球，即本键出现之前的画面。负值按 0 处理。详见下方 `globe` 镜头一节。 |
...
- **背面**（`globe_back_fade`）：每个像素的视线与球相交两次。默认只显示近交点；`globe_back_fade > 0` 时，远交点的光以**叠加**方式加到该像素上（晕是发光体，正面不遮挡背面），权重为 `1 − smoothstep(0, globe_back_fade, depth)`，`depth` 是远交点比轮廓离相机多出的距离。权重在球缘处为 1，背面与正面连续衔接，到 `depth = globe_back_fade` 处降为 0。只叠加背面的光：网格线、地平线、marker 与标签只画在正面。曝光不受影响（背面的光不计入落地能量）。GUI 预览与 CLI 使用同一权重，公式位于 `lm_proj::GlobeBackFadeWeight`（`src/core/shared/projection_shared.h`）。

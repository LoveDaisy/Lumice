...
- **背面**（`globe_back_fade`）：每个像素的视线与球相交两次。默认只显示近交点；`globe_back_fade > 0` 时，远交点的光以**叠加**方式加到该像素上（晕是发光体，正面不遮挡背面），权重为 `exp(−depth / globe_back_fade)`（指数雾），`depth` 是远交点比轮廓离相机多出的距离。权重在球缘处为 1，背面与正面连续衔接，在 `depth = globe_back_fade` 处为 `e⁻¹ ≈ 0.37`，且永远不会精确到 0——刻意不设截止。背面的**线**——网格（`elevation` / `longitude`）、`horizon`、`angular_dist` 与 `view_dist`——也画在远交点处，每条线的不透明度乘以同一权重，并且位于正面线之下：两者重叠的像素上正面线胜出。marker（`zenith_nadir`、`markers`）与文字标签仍只画在正面。曝光不受影响（背面的光不计入落地能量）。GUI 预览与 CLI 使用同一权重，公式位于 `lm_proj::GlobeBackFadeWeight`（`src/core/shared/projection_shared.h`）。

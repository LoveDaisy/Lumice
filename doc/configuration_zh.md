...
>
> **计入 `ray_num` 的光线并不都会进入晶体。** 分给某颗晶体的光线以概率 `A/(S/2)`（晶体沿光线方向
> 的投影面积除以其表面积的一半）保留，否则在追迹前丢弃；被丢弃的光线仍计入 `ray_num` 与发射能量。
> 对随机取向的晶体约有一半光线被保留，因此同一 `ray_num` 下图像比这个数字所暗示的更嘈杂。这对
...
> 光线方向的投影面积除以其表面积的一半。对随机取向的晶体，这个个数份额也恰好就是该条目的
...
> - **噪声**：分给随机取向晶体的光线约一半（不论形状，平均恰为 1/2）、分给定向晶体的光线按场景而定
>   的一部分，现在会在入射时被拒，因此同一 `ray_num` 下每张画面都更嘈杂。把 `ray_num` 翻倍可让随机
>   取向场景回到原来的噪声水平。
> - **整体亮度**：默认的 `ev_mode: relative` 下按新画面重新测光，不会变暗。`ev_mode: absolute` 下被拒
>   光线仍计入发射，同一 EV 画面会变暗：随机取向晶体恰好暗一档（EV 加 1，或 `intensity_factor` ×2，

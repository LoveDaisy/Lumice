...
>
> **计入 `ray_num` 的光线并不都以全权重进入晶体。** 分给某颗晶体的光线带着因子 `A/(S/2)`（晶体沿
> 光线方向的投影面积除以其表面积的一半）进入晶体，这个因子按期望施加，各路线施加方式不同：CPU 路线
> （默认）以概率 `A/(S/2)` 保留光线、否则在追迹前丢弃；GPU 路线（Metal、CUDA）追迹每条光线，改为把
> 它的权重乘以 `A/(S/2)`。两者给出的图像期望相同。无论哪种，损失的部分都仍计入 `ray_num` 与发射
> 能量——CPU 上是被丢弃的光线，GPU 上是被保留光线在入射处减去的权重。对随机取向的晶体该因子平均
> 恰为 1/2，因此 CPU 路线上约有一半光线被保留，同一 `ray_num` 下图像比这个数字所暗示的更嘈杂。这对
...
> 光线方向的投影面积除以其表面积的一半（CPU 路线上作为保留概率、GPU 路线上作为权重，见上一条说明）。对随机取向的晶体，这个个数份额也恰好就是该条目的
...
> - **噪声**随路线而异。CPU 路线（默认）上，分给随机取向晶体的光线约一半（不论形状，平均恰为 1/2）、
>   分给定向晶体的光线按场景而定的一部分，现在会在入射时被拒，因此同一 `ray_num` 下每张画面都更嘈杂；
>   把 `ray_num` 翻倍可让随机取向场景回到原来的噪声水平。GPU 路线（Metal、CUDA）上没有光线被拒——
>   每条都以权重 `A/(S/2)` 追迹——同一 `ray_num` 下绝不比拒绝更嘈杂：单波长实测逐像素方差约为拒绝的
>   一半；默认 D65 光谱下二者差异在测量噪声之内。CPU 保留拒绝，是因为被拒光线对它几乎零成本，而
>   追迹全部光线在 CPU 上实测单位噪声的吞吐更低。
> - **整体亮度**：默认的 `ev_mode: relative` 下按新画面重新测光，不会变暗。`ev_mode: absolute` 下被去掉
>   的部分——CPU 路线上被拒的光线、GPU 路线上光线在入射处减去的权重——仍计入发射，同一 EV 画面会
>   变暗，各路线幅度相同：随机取向晶体恰好暗一档（EV 加 1，或 `intensity_factor` ×2，

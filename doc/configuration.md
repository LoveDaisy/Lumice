...
> it with its weight multiplied by `A/(S/2)` — the crystal's projected area along the ray over
> half its surface area. Every ray is traced, on every route (CPU, Metal, CUDA); none is
> discarded. The weight a ray loses at entry still counts toward the emitted energy, and every
> dealt ray counts toward `ray_num`. For randomly oriented crystals the factor averages exactly
> 1/2. See the `ray_allocation` note below for what this means for `proportion`, and the
> migration note after it for what it changed.
...
> (applied as a weight on every ray; the note above). For
...
> - **Noise**: no ray is rejected — every ray is traced, at weight `A/(S/2)` — so the number
>   of rays behind a picture does not drop. Where the area a crystal shows the sun varies
>   across its sampled orientations the weights vary with it, which adds some per-pixel noise
>   at the same `ray_num`; measured at a single wavelength, that is about half the per-pixel
>   variance of the alternative of discarding rays with probability `1 - A/(S/2)`.
> - **Overall brightness**: under the default `ev_mode: relative` it is re-metered on the new
>   picture and does not drop. Under `ev_mode: absolute` the weight a ray loses at entry still
>   counts as emitted, so the picture darkens at the same EV: exactly one stop for randomly oriented
...
| `visible` | string | no | "upper" | Visible hemisphere: "upper", "lower", or "full". A **display clip**, not an energy cull, and the same statement for all four lens families (single-lens `linear`/`fisheye_*`, `rectangular`, dual-fisheye, `globe`): rays always accumulate into the full-sky buffer, and the excluded region is dropped afterwards, on the way to pixels — so it renders as neither background nor ray energy. Do not read it as a throughput knob: it does not reduce the work done per ray. Exposure is metered **before** the clip and is therefore unaffected by this key; that has a brightness consequence for pre-existing single-lens scenes, spelled out in [`doc/ev-pipeline-architecture.md`](ev-pipeline-architecture.md) §2.7. Not to be confused with the structural clipping a projection does on its own (a direction behind an orthographic fisheye's camera has no pixel to land on at all); that is the projection's domain, not this key. |
...
| `globe_back_fade` | float | no | 0.0 | `globe` lens only (every other lens ignores it): the fog length of its **far side**, seen through the near side and fading like fog with distance from the camera: the far side's weight is `exp(−depth / globe_back_fade)`, so a larger value fades more slowly and nothing ever fades to exactly 0. In the globe's own units (unit sphere, camera `D = 4` away), so the far side's deepest point — straight behind the centre — sits `(D+1) − √(D²−1) ≈ 1.127` behind the silhouette; the GUI slider runs 0–2, where 0.3–0.6 reads as a light haze and 2 already shows the deepest point at ~0.57. `0` shows the camera-facing hemisphere only, which is the look before this key existed. Negative values are clamped to 0. See the `globe` lens section below. |
| `display_mode` | string | no | `"normal"` | What the finished image is **shown as**. `"normal"`: the image itself. `"channel_br"`: the "bluer or redder here" diagnostic — each pixel becomes grey `clamp(0.5 + 0.5·(B − R), 0, 1)` of the displayed sRGB channels (after gamma, the same subtraction an image editor's channel calculator does): mid grey = no difference, lighter = bluer, darker = redder. The value moves with exposure and flattens where R or B clips, so compare places at the same EV. Inert under `"tone": "print"`; while it is on, a colour-classed scene produces no raypath composite, and a non-default `ray_color` makes the diagnostic meaningless (it shows the tint's constant offset); each is logged as a warning at commit. Overlays are drawn on top unchanged. An unknown value falls back to `"normal"` with a warning. Formula: `src/util/channel_math.hpp`. |
...
- **Far side** (`globe_back_fade`): each pixel's ray crosses the sphere twice. By default only the near crossing is shown. With `globe_back_fade > 0` the far crossing's light is **added** to the pixel (a halo is emitted light, so the near side does not hide it), weighted by `exp(−depth / globe_back_fade)` (exponential fog), where `depth` is how much further the far point is from the camera than the silhouette. The weight is 1 at the rim, so the far side joins the near side continuously, is `e⁻¹ ≈ 0.37` at `depth = globe_back_fade`, and never reaches exactly 0 — there is deliberately no cutoff. The far side's **curves** — grid (`elevation` / `longitude`), `horizon`, `angular_dist` and `view_dist` — are drawn too, at the far point, with each line's opacity multiplied by the same weight, and underneath the near side's lines, so a near-side line always wins a pixel both touch. Markers (`zenith_nadir`, `markers`) and text labels stay on the near side alone. `visible` and `front` clip the two crossings **each by its own direction**: the near and far points of one pixel generally lie at different altitudes, so under `visible: upper` a far point below the horizon adds neither light nor lines even when the near point in front of it is kept, and a far point above the horizon still shows through a near point that is clipped. The sky `background` belongs to the near side: a pixel whose near point is clipped shows the far side's light over black (or bare `paper` under `tone: print`), never the sky colour. The raw XYZ export (`--format npy`, the C API) is not a display and keeps both sides' energy regardless. Known gap: with `raypath_color` the composite image still clips a globe pixel by its near point alone. Exposure is unaffected (far-side light does not count toward the landed energy). The GUI preview and the CLI apply the same weight; the formula lives in `lm_proj::GlobeBackFadeWeight` (`src/core/shared/projection_shared.h`).
- `.lmc` compatibility: the only field added is `globe_back_fade` (default 0); older `.lmc` files load unchanged.

...
| `globe_back_fade` | float | no | 0.0 | `globe` lens only (every other lens ignores it): how far behind the sphere's silhouette its **far side** stays visible, seen through the near side and fading out with distance from the camera. In the globe's own units (unit sphere, camera `D = 4` away), so the far side's deepest point — straight behind the centre — sits `(D+1) − √(D²−1) ≈ 1.127` behind the silhouette; `0` shows the camera-facing hemisphere only, which is the look before this key existed. Negative values are clamped to 0. See the `globe` lens section below. |
...
- **Far side** (`globe_back_fade`): each pixel's ray crosses the sphere twice. By default only the near crossing is shown. With `globe_back_fade > 0` the far crossing's light is **added** to the pixel (a halo is emitted light, so the near side does not hide it), weighted by `1 − smoothstep(0, globe_back_fade, depth)`, where `depth` is how much further the far point is from the camera than the silhouette. The weight is 1 at the rim, so the far side joins the near side continuously, and fades to 0 at `depth = globe_back_fade`. Only the far side's light is added: grid lines, the horizon, markers and labels are drawn on the near side alone. Exposure is unaffected (far-side light does not count toward the landed energy). The GUI preview and the CLI apply the same weight; the formula lives in `lm_proj::GlobeBackFadeWeight` (`src/core/shared/projection_shared.h`).

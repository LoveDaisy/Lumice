...
- **Core / rendering architecture**: `accumulator-consumer-architecture.md`, `raypath-rayseg-architecture.md`, `raypath-symmetry.md`, `coordinate-convention.md`, `crystal-orientation-sampling.md`, `ev-pipeline-architecture.md`, `adaptive-brightness.md`, `filter-architecture.md`
  ...
  - `raypath-analysis-panel.md` — **光路分析面板设计记录（as-built，2026-09-11，`scrum-raypath-analysis-panel`
    #535 五子任务全部落地；分支 `feat/raypath-analysis-panel`，未合并 `main`）**：
    从 backlog「光路成分分析」(ii) 剥离需求的内存墙拆出第三种形态——**专用统计运行**（不渲染、只按完整光路聚合能量）
    ...
    ⭐**唯一遗留的产品级开放点**：chain id 约化对称来源采用「会话级统一标志」（候选 B，默认
    P|B|D）而非「复用晶体上恰好一条 symmetry 的 filter」（候选 A），owner 尚未显式签字，见 §7 末尾。

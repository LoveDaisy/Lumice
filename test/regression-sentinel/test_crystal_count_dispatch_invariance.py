...
3. **Worker invariance.** The CPU route runs a multi-worker pool by default
   (the platform's core count, capped), each worker owning its own Simulator and
...
# machine's core count, so a hardcoded larger value would be untestable on small

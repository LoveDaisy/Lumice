...
// which turns "remember to change both" into a red test rather than a hope.
//
// It used to have a neighbour, kEvAutoDownsampleFactor, mirroring the mono auto-EV's box-sum
// factor for the same no-core-includes reason (src/gui/ may not include core/ — see AGENTS.md).
// That one is gone: the GUI no longer computes an anchor, it reads the server's, and a
// downsample factor is now an implementation detail of the anchor plane that no consumer needs
// to know. One fewer mirrored constant is the point rather than a side effect — see
// core/ev_anchor.hpp for the core-side half of the same removal.

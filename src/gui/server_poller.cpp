...
void ServerPoller::PollOnce() {
  ...
  if (has_new_snapshot || force_final_upload) {
    ...
    unsigned long long calibrated = calibrated_min_rays_.load();
    unsigned long long min_rays = calibrated != 0 ? calibrated : gui::kMinRaysFloor;
    ...
    if (quality_ok) {
      ...
      payload->axis_solid_angle = xyz_results[0].axis_solid_angle;

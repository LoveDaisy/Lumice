    ...
    uint tid [[thread_position_in_grid]])
{
  ...
  // × the projected-area entry weight A / (S/2) (entry_accept_prob, pcg_shared.h
  ...
  float weight = wl_pool[wl_idx].spd_weight * gp.alloc_correction * entry_accept_prob(proj_sum, s_total);
    ...
    uint tid [[thread_position_in_grid]])
{
  ...
  float w = cont_w_in[tid] * gp.alloc_correction * entry_accept_prob(proj_sum, s_total);
  if (to_face == kInvalidId) {
    w = 0.0f;
  }

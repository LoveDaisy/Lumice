...
// Sample crystal origin (p, from_face_, to_face_) and apply the projected-area
// entry factor (keep/discard + weight). Direction d and the birth weight w MUST
...
// `entry_keep_floor`: see InitRay_p_fid.
void InitRayOtherMs(RandomNumberGenerator& rng, const RayBuffer init_data[2], size_t curr_ray_num,
                    ...
                    float entry_keep_floor, RayBuffer buffer_data[2], RayBuffer& all_data, size_t& init_ray_offset,
                    float weight_correction = 1.0f);

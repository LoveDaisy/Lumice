...
ChainIdInterningTable::ChainIdInterningTable(size_t capacity) : capacity_(capacity) {
...
uint32_t ChainIdInterningTable::Intern(uint32_t parent_id, IdType crystal_id, std::vector<IdType> segment) {
  if (parent_id == kOverflowChainId) {
    // Sticky: the chain was lost at an earlier layer, and a real entry under
    // the sentinel would be a node with no path to the root. Not counted —
    // the loss was counted where it happened.
    return kOverflowChainId;
  }
...
size_t ChainIdInterningTable::ConsumeOverflowCount() {
  const size_t n = overflow_since_flush_;
...
std::vector<uint32_t> ChainIdInterningTable::PathToRoot(uint32_t id) const {
  ...
  if (id == kOverflowChainId) {
    return path;  // no entry, no parent chain: nothing to walk
  }
...
uint32_t ChainIdMerger::Resolve(uint32_t producer_key, uint32_t local_id) const {
  if (local_id == ChainIdInterningTable::kRootChainId || local_id == ChainIdInterningTable::kOverflowChainId) {
    return local_id;
  }
